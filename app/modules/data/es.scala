...
import common.tools.IdGenerator
...
object ElasticsearchClient {
  ...
  val actorName = current.configuration.getString("app.es.actorName").getOrElse("ElasticSearchClientActor")
  val actorTimeout = current.configuration.getMilliseconds("app.es.actorTimeout").map(Timeout(_, TimeUnit.MILLISECONDS)).getOrElse(Timeout(11, TimeUnit.SECONDS))
  val wsTimeout = current.configuration.getMilliseconds("app.es.wsTimeout").map(Timeout(_, TimeUnit.MILLISECONDS)).getOrElse(Timeout(10, TimeUnit.SECONDS))
  val esUrl = current.configuration.getString("app.es.esUrl").getOrElse("http://localhost:9200")
  val esIndex = current.configuration.getString("app.es.esIndex").getOrElse("app")
